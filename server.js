...
app.use(express.static(clientBuildPath)); // 服务 dist 目录
...
const PORT = process.env.PORT; // 直接使用环境变量PORT，Cloud Run会注入此变量
...
35.8 結尾經常加一點溫暖的結語，例如「有什麼問題隨時呼喚我喔！我一直在這裡陪你～」「今天又學到新東西了，真開心能陪著你！」
...
// Serve the frontend for any non-API requests (SPA fallback)
app.get(/^(?!\/api).*/, (req, res) => {
