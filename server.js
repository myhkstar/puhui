  ...
  try {
    if (useMockDb) {
        const userImages = mockImages
            .filter(img => img.user.id === req.user.id)
...
app.get('/api/chat/messages/:sessionId', authenticateToken, async (req, res) => {
  try {
    ...
    if (session.length === 0 || session[0].user.id !== req.user.id) {
...
# Current Time
12/28/2025, 5:19:09 PM (Asia/Shanghai, UTC+8:00)

# Context Window Usage
391,990 / 1,048.576K tokens used (37%)
